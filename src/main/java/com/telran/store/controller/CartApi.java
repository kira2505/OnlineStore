...
public interface CartApi {
    ...
    @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Cart item details", required = true,
            content = @Content(schema = @Schema(implementation = AddToCartRequestDto.class),
                    examples = @ExampleObject(value = "{ \"productId\": 3, \"quantity\": 2 }")))
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Invalid input data",
                    content = @Content(examples = @ExampleObject(value = "Quantity must be at least 1"))),
            @ApiResponse(responseCode = "404", description = "Products not found",
                    content = @Content(examples = @ExampleObject(value = "Product with ID 3 not found")))
    })
    ...
    @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Update cart item details", required = true,
            content = @Content(schema = @Schema(implementation = AddToCartRequestDto.class),
                    examples = @ExampleObject(value = "{ \"productId\": 3, \"quantity\": 5 }")))
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Successfully added product to cart",
                    content = @Content(schema = @Schema(implementation = CartItemResponseDto.class))),
            @ApiResponse(responseCode = "400", description = "Invalid product ID or quantity",
                    content = @Content(examples = @ExampleObject(value = "Quantity must be a positive number"))),
            @ApiResponse(responseCode = "404", description = "Products not found",
                    content = @Content(examples = @ExampleObject(value = "Product with ID 3 not found in cart")))
    })
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404", description = "Cart not found",
                    content = @Content(examples = @ExampleObject(value = "Cart is empty or does not exist")))
    })
